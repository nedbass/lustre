...
# LB_LDISKFS_EXT_RELEASE
#
# Detect if ext3 or ext4 sources should be used for ldiskfs.
#
...
if test -e "$linux_src/fs/$LDISKFS_BACKFS/super.c"; then
	...
	if test -e "$linux_src/fs/$LDISKFS_BACKFS/super.c"; then
		...
		AC_MSG_ERROR([Could not locate $LDISKFS_BACKFS source.])
	fi
fi
...
if test $LDISKFS_BACKFS = 'ext4'; then
	LB_LDISKFS_WALK_SPACE_HAS_DATA_SEM
	AC_DEFINE(HAVE_EXT4_LDISKFS, 1, [build ext4 based ldiskfs])
fi

LB_LDISKFS_SINGLEDATA_TRANS_BLOCKS_SB
LB_LDISKFS_DISCARD_PREALLOCATIONS
LB_LDISKFS_INSERT_EXTENT_WITH_5ARGS
...
# LB_LDISKFS_SINGLEDATA_TRANS_BLOCKS_SB
#
AC_DEFUN([LB_LDISKFS_SINGLEDATA_TRANS_BLOCKS_SB],
[
AC_MSG_CHECKING([if LDISKFS_SINGLEDATA_TRANS_BLOCKS takes sb as argument])
tmp_flags="$EXTRA_KCFLAGS"
EXTRA_KCFLAGS="-I$EXT_DIR/.. -I$LDISKFS_DIR"
...
# LB_LDISKFS_DISCARD_PREALLOCATIONS
#
AC_DEFUN([LB_LDISKFS_DISCARD_PREALLOCATIONS],
[
...
LB_LINUX_TRY_COMPILE([
        ...
        AC_DEFINE(LDISKFS_DISCARD_PREALLOCATIONS, 1,
                  [ldiskfs_discard_preallocations defined])
],[
        AC_MSG_RESULT(no)
])
EXTRA_KCFLAGS="$tmp_flags"
...
# LB_LDISKFS_INSERT_EXTENT_WITH_5ARGS
#
AC_DEFUN([LB_LDISKFS_INSERT_EXTENT_WITH_5ARGS],
[
...
# LB_LDISKFS_WALK_SPACE_HAS_DATA_SEM
#
# 2.6.32 ext4_ext_walk_space() takes i_data_sem internally.  This check
# is only relevant when building lustre with an ext4 based ldiskfs.
#
AC_DEFUN([LB_LDISKFS_WALK_SPACE_HAS_DATA_SEM],
[
LB_CHECK_FILE([$EXT_DIR/extents.c], [], [ AC_MSG_ERROR(
	[$LDISKFS_BACKFS extents.c source must exist for lustre build])])
