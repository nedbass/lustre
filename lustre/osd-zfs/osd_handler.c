 ...
 */
static void
osd_object_sa_dirty_rele(struct osd_thandle *oh)
{
 ...
 */
static int
osd_object_sa_update(struct osd_object *obj, sa_attr_type_t type,
...
{
        struct osd_object  *obj = osd_dt_obj(dt);
        struct osd_device  *od  = osd_obj2dev(obj);
        struct osd_thandle *oh;
        int rc = 0;
        ENTRY;
...
static void osd_xattr_changed_cb(void *arg, uint64_t newval)
{
        ...
        o->od_xattr_in_sa = (newval == ZFS_XATTR_SA) ? 1 : 0;
}
